...
/**
 * Created by hnguyen on 12/12/15.
 */
...
public class Glyphs {
    ...
    private int width = 16;	// width in pixels of one character
    private int height = 18;	// height in pixels of one character
    ...
    public Glyphs(Bitmap bitmap, int w, int y) {
        super();
        this.bitmap = bitmap;
//        this.width = w;
//        this.height = y;
     ...
     * @param text
     */
    public void drawChar(Canvas canvas, Character c, int x, int y) {
        if (canvas == null) {
            Log.d(TAG, "Canvas is null");
        }
        if (glyphs.get(c) != null) {
            canvas.drawBitmap(glyphs.get(c), x, y, null);
        }
    }
}
